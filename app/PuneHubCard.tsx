import { Building2, Plane, Laptop2, TrainFront } from "lucide-react";

type HubItem = {
  title: string;
  subtitle: string;
  icon: "building" | "plane" | "laptop" | "train";
  className: string;
...
const hubItems: HubItem[] = [
  { title: "Baner", subtitle: "IT Hub", icon: "building", className: "left-[12%] top-[22%]" },
  { title: "Viman\nNagar", subtitle: "Airport Area", icon: "plane", className: "left-1/2 top-[8%] -translate-x-1/2", sizeClassName: "h-[92px] w-[92px] sm:h-[108px] sm:w-[108px]", titleClassName: "text-[0.66rem] sm:text-[0.8rem]" },
  { title: "Kharadi", subtitle: "EON IT Park", icon: "building", className: "right-[12%] top-[22%]" },
  { title: "Hadapsar /\nMagarpatta", subtitle: "Magarpatta City", icon: "building", className: "left-[18%] bottom-[12%]", sizeClassName: "h-[102px] w-[102px] sm:h-[120px] sm:w-[120px]", titleClassName: "text-[0.62rem] sm:text-[0.74rem]", subtitleClassName: "text-[0.46rem] sm:text-[0.54rem]" },
  { title: "Shivajinagar /\nPune Station", subtitle: "Railway Station", icon: "train", className: "right-[15%] bottom-[12%]", sizeClassName: "h-[104px] w-[104px] sm:h-[122px] sm:w-[122px]", titleClassName: "text-[0.62rem] sm:text-[0.74rem]", subtitleClassName: "text-[0.46rem] sm:text-[0.54rem]" },
];

function HubIcon({ icon }: { icon: HubItem["icon"] }) {
  const className = "h-6 w-6 text-[#ff6d16] sm:h-7 sm:w-7";

  if (icon === "plane") return <Plane className={className} strokeWidth={2.2} />;
...
export default function PuneHubCard() {
  return (
    <div className="relative overflow-hidden rounded-[1.75rem] bg-[radial-gradient(circle_at_center,rgba(255,140,40,0.16),transparent_38%),linear-gradient(135deg,#ff6c0f_0%,#ff6508_38%,#f15908_72%,#e3520b_100%)] p-4 text-white shadow-[0_24px_80px_-42px_rgba(249,115,22,0.85)] sm:p-5">
      ...
      <div className="relative min-h-[390px] sm:min-h-[430px]">
        ...
        <div className="absolute left-1/2 top-1/2 z-10 flex h-[128px] w-[128px] -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border-[4px] border-[#ffb682] bg-[radial-gradient(circle_at_35%_30%,#fffaf5_0%,#fff4eb_100%)] text-[2.15rem] font-semibold text-[#ff6408] shadow-[0_30px_60px_-24px_rgba(255,255,255,0.35)] sm:h-[146px] sm:w-[146px] sm:text-[2.45rem]">
          Pune
        </div>
