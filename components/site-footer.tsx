...
import { contactDetails, footerLocationGroups, SITE_NAME, SITE_TAGLINE } from "@/data/site";
    ...
    <footer className="bg-zinc-950 pb-24 pt-14 text-white md:pb-10">
      <Container>
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          {/* Contact card */}
          <div className="min-w-0 rounded-2xl border border-white/10 bg-white/5 p-5 sm:p-6">
            ...
            <div className="mt-5 space-y-4 border-t border-white/10 pt-4 text-sm text-zinc-300">
              <div className="flex flex-col gap-2">
                <div className="row-inline max-w-full gap-3">
                  <a href={`tel:+${contactDetails.phoneRaw}`} className="inline-flex items-center gap-2 whitespace-nowrap rounded-full border border-white/15 px-4 py-2 text-sm font-medium text-white transition hover:border-accent-300/50">
                    <Phone className="h-3.5 w-3.5" />
                    {contactDetails.phoneDisplay}
                  </a>
                  <a href={`tel:+${contactDetails.secondaryPhoneRaw}`} className="inline-flex items-center gap-2 whitespace-nowrap rounded-full border border-white/15 px-4 py-2 text-sm font-medium text-white transition hover:border-accent-300/50">
                    {contactDetails.secondaryPhoneDisplay}
                  </a>
                  <a href={`https://wa.me/${contactDetails.whatsappRaw}`} className="inline-flex items-center gap-2 whitespace-nowrap rounded-full bg-green-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-green-500">
          ...
          <div className="min-w-0 rounded-2xl border border-white/10 bg-white/5 p-5 sm:p-6">
            ...
            <p className="mt-1 text-sm text-zinc-400">Browse pickup coverage across Pune and PCMC with dedicated area pages.</p>
            <div className="mt-5 grid gap-5 sm:grid-cols-2">
              {footerLocationGroups.map((group) => (
                <div key={group.title}>
                  <h4 className="text-sm font-semibold uppercase tracking-[0.2em] text-accent-300">{group.title}</h4>
                  <div className="mt-3 space-y-2">
                    {group.locations.map((location) => (
                      <Link
                        key={location.route}
                        href={`/${location.route}`}
                        className="flex w-full min-w-0 items-center gap-2 rounded-xl border border-white/8 bg-white/5 px-3 py-2.5 text-sm text-zinc-300 transition hover:border-accent-300/40 hover:text-white"
                      >
