  ...
  return (
    <>
      <div className="rounded-[2rem] border border-zinc-200 bg-white p-4 shadow-soft sm:p-5">
        ...
        <div ref={railRef} className="no-scrollbar mt-6 flex snap-x snap-mandatory gap-4 overflow-x-auto pb-2">
          {items.map((item, index) => (
            <motion.button
              ...
              className="group relative min-w-[82vw] snap-center overflow-hidden rounded-[1.9rem] border border-zinc-200 bg-white text-left shadow-soft sm:min-w-[460px] xl:min-w-[560px]"
            >
        ...
        <div className="row-scroll mt-5 flex gap-3 pb-1">
          {items.map((item, index) => (
            <button
              key={`${item.id}-thumb`}
              type="button"
